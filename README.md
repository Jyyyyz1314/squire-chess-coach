...
- 公开多用户 BYOK：每位访客在网页中填写自己的 OpenAI 兼容 API，配置互不共享；每位访客也可独立授权自己的 Lichess。
- 导入普通或带注释/变例的 Lichess PGN，也可直接粘贴棋谱文本。
- 导出/导入 Squire JSON 学习档案，保留逐步笔记且不保存模型回答。
- 10 个经典开局课程、每课 11 条深度变例（共 110 条、每条至少 12 回合）：提供固定中文理论、逐步意图、典型计划与可追溯资料；完成进度按变例与执棋方保存在本机，连接个人 API 后每一步都会获得经校验的 AI 深入反馈。
- 闯关式学习地图、每日新课与复习目标、XP 等级、连续学习天数，以及按掌握度自动延长的间隔复习队列。
- Lichess OAuth2 + PKCE 安全登录、近期棋局同步、主动性/战术活跃/王的安全/开局纪律/行棋稳定/残局经验六维画像与一键复盘。
- 服务端输入校验、同源检查、公网 HTTPS 地址校验、超时重试和基础限流；老师回答不做跨用户缓存。
...
不配置模型也可以使用棋盘、PGN、笔记、固定开局课程和 Stockfish。AI 老师由使用者在网页右上角 **AI 设置** 中填写自己的 API 地址、Key 和模型名；默认只保存到当前浏览器会话，用户明确勾选后才会持久保存在该设备。

服务端 `.env.local` 只保留功能开关和基础限流：
...
用户 Key 会随主动发起的讲解请求经过同源服务端代理转发给用户指定的模型供应商，只在请求内存中短暂存在，不写入数据库、日志、棋谱或回答缓存。自定义 API 地址必须是 HTTPS 公网地址。
