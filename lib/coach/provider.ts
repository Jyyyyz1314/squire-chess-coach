...
import { CoachConfig, CoachCredentials, normalizeCoachEndpoint } from './config';
export const CoachRequest = z.object({ mode: z.enum(['quick', 'deep', 'question', 'training']).default('quick'), fen: z.string().max(120), sideToMove: z.enum(['白方', '黑方']), question: z.string().trim().min(1).max(500), opening: z.string().max(200), playedMove: z.string().max(20).optional(), lines: z.array(z.object({ rank: z.number(), depth: z.number(), score: z.number(), mate: z.number().optional(), pv: z.array(z.string().max(12)).max(100) })).max(3) });
export const CoachApiRequest = CoachRequest.extend({ config: CoachCredentials });
export interface CoachProvider { explain(context: z.infer<typeof CoachRequest>, config: CoachConfig): Promise<string> }
...
export class ApiCoach implements CoachProvider {
  async explain(context: z.infer<typeof CoachRequest>, config: CoachConfig) {
    const position = new Chess(context.fen);
    const normalizedContext = { ...context, sideToMove: position.turn() === 'w' ? '白方' as const : '黑方' as const };
    const { apiKey: key, model } = config;
    const endpoint = normalizeCoachEndpoint(config.baseUrl);
    const payload: Record<string, unknown> = { model, messages: [{ role: 'system', content: COACH_SYSTEM_PROMPT }, { role: 'user', content: `${coachTask(normalizedContext.mode)}\n\n局面数据：\n${JSON.stringify(normalizedContext)}` }], max_tokens: normalizedContext.mode === 'deep' ? 900 : normalizedContext.mode === 'question' || normalizedContext.mode === 'training' ? 760 : 260, stream: true };
    ...
    if (normalizedContext.mode !== 'question' && normalizedContext.mode !== 'training') return draft;
